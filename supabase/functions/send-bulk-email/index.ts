...
// ✅ Single source of truth for CORS headers — shared across ALL responses
const corsHeaders = {
...
serve(async (req) => {
  // ✅ OPTIONS preflight — must return 200 with cors headers (not null body)
  ...
  try {
    ...
    await updateSendRunStatus(supabase, sendRun.id, "completed", null, sentCount, failedCount);
    ...
    return new Response(
      ...
      // ✅ catch-all error also needs cors headers or the browser still blocks it
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
});
